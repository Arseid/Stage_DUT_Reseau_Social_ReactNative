<?php
    ...
    // -1 follower
    $SRTargetUser="SELECT * from profile WHERE user_id='$self_user_id'";
    $SQTargetUser=mysqli_query($ConnectDB,$SRTargetUser);
    $rowTargetUser=mysqli_fetch_row($SQTargetUser);

    $arrayFollower=explode(',',$rowTargetUser[5]);
    $key=array_search($target_user_id,$arrayFollower);
    array_splice($arrayFollower,$key,1);
    $arrayFollowerAfter=implode(',',$arrayFollower);

    $SRTargetUser="UPDATE profile SET followers = '$arrayFollowerAfter' WHERE profile.user_id = '$self_user_id'";
    $SQTargetUser=mysqli_query($ConnectDB,$SRTargetUser);

    // -1 following
    $SRUser="SELECT * from profile WHERE user_id='$target_user_id'";
    $SQUser=mysqli_query($ConnectDB,$SRUser);
    $rowUser=mysqli_fetch_row($SQUser);

    $arrayFollowing=explode(',',$rowUser[6]);
    $key=array_search($self_user_id,$arrayFollowing);
    array_splice($arrayFollowing,$key,1);
    $arrayFollowingAfter=implode(',',$arrayFollowing);

    $SRUser="UPDATE profile SET following = '$arrayFollowingAfter' WHERE profile.user_id = '$target_user_id'";
    $SQUser=mysqli_query($ConnectDB,$SRUser);
      
    echo json_encode('Follower Removed');
?>